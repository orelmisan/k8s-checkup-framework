...
import (
	...
	"github.com/orelmisan/k8s-checkup-framework/checkups/kubevirt-vm-latency/tests/libvmi"
)
...
func LoginToFedora(vmi *v1.VirtualMachineInstance) error {
	...
	if err != nil {
		kvlog.DefaultLogger().Object(vmi).Reason(err).Errorf("Login attempt failed: %+v", res)
		...
		if err != nil {
			kvlog.DefaultLogger().Object(vmi).Reason(err).Errorf("Retried login attempt after two minutes failed: %+v", res)
			return err
		}
	}
...
func configureConsole(expecter expect.Expecter, shouldSudo bool) error {
	...
	if err != nil {
		kvlog.DefaultLogger().Infof("%v", resp)
	}
	return err
}
