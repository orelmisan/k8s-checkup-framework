...
// NOTE: there is a safer version that validates sent commands `SafeExpectBatch` refer to it about limitations.
func ExpectBatch(vmi *v1.VirtualMachineInstance, expected []expect.Batcher, timeout time.Duration) ([]expect.BatchRes, error) {
...
func RunCommand(vmi *v1.VirtualMachineInstance, command string, timeout time.Duration) ([]expect.BatchRes, error) {
	resp, err := ExpectBatch(vmi, []expect.Batcher{
		&expect.BSnd{S: "\n"},
